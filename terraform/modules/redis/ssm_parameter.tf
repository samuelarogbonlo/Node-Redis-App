...
resource "aws_ssm_parameter" "elasticache_ep" {
  name   = "/elasticache/node-app/${aws_elasticache_replication_group.app4.replication_group_id}/endpoint"
  type   = "SecureString"
...
resource "aws_ssm_parameter" "elasticache_port" {
  name   = "/elasticache/node-app/${aws_elasticache_replication_group.app4.replication_group_id}/port"
  type   = "SecureString"
  key_id = aws_kms_key.encrytion_rest.id
